...
conf t
 ...
 int gi0/1
  ...
  exit
 ip routing
 ip route 0.0.0.0 0.0.0.0 10.92.92.1 120
 enable secret pass
 line vty 0 14
  password pass
  transport input all
...
conf t
 int fa0/0
  ...
  exit
 ip routing
 ip route 0.0.0.0 0.0.0.0 10.92.100.4 120
 enable secret pass
 line vty 0 14
  password pass
  transport input all
...
conf t
 int gi 0/0/0
  ...
  exit
 ip routing
 ip route 10.92.0.0 255.255.0.0 10.92.92.4 120
 enable secret pass
 line vty 0 14
  password pass
  transport input all
...
| Description                      | Command                                  |
| ---                              | ---                                      |
| a non-default hostname           | hostname coreBaba92                  |
...
ifconfig ens256 10.92.1.6 netmask 255.255.255.0 up
~~~
...
ifname ens256 \
ipv4.method manual \
ipv4.addresses 10.92.1.6/24 \
autoconnect yes

nmcli connection up BRIDGED

ip route add 10.0.0.0/8 via 10.92.1.4 dev ens256
ip route add 200.0.0.0/24 via 10.92.1.4 dev ens256
...
~~~
!@NetOps
ansible-playbook -i rivan_mkt.ini playbooks/deploy_92.yml --skip-tags ivrs
~~~
...
dial-peer voice 69 voip
 ...
 session target ipv4:10.92.100.1
